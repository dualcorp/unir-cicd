...
test-unit:
	mkdir -p results
	docker run --rm --volume ./:/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest pwd || true
	docker run --rm --volume ./:/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest ls -ltr || true
